...
```{r}
test_prop <- 0.3 # 30% des donnees dans un ensemble test
...
```{r}
ap_dtm_train_filtreSparse <- ap_dtm_train_filtreSparse[-493,]  # cf tableau avec which(ind_rows_0 = F)
ind_rows_0 <- rowSums(as.matrix(ap_dtm_train_filtreSparse))!=0 
table(ind_rows_0) 
...
# on utilise la LDA pour extraire les etiquettes (pour utiliser par la suite dans Random Forest)
